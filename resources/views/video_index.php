            ...
            <div class="video-col col-sm-5">
               <div class="image-holder">
                <span data-picture data-alt="image description">
                  ...
                  <span data-src="<?php echo $video->thumbnail_url; ?>" data-height="300" data-width="458" data-media="(max-width:1023px)" ></span> <!-- retina 1x tablet -->
                  <span data-src="<?php echo $video->thumbnail_url; ?>" data-height="300" data-width="458" data-media="(max-width:1023px) and (-webkit-min-device-pixel-ratio:1.5), (max-width:1023px) and (min-resolution:144dpi)" ></span> <!-- retina 2x tablet -->
                  <noscript><img src="<?php echo $video->thumbnail_url; ?>" height="300" width="458" alt="image description"></noscript>
                </span>
      ...
      <?php if(isset($zype_pagination) && ($zype_pagination->previous || $zype_pagination->next)){ ?>
        <ul class="pagination">
          <?php if($zype_pagination->previous){ ?>
            <li class="next">
              <a href="<?php zype_url('video'); ?>/page-<?php echo $zype_pagination->previous.zype_search_and_sort_querystring(); ?>" aria-label="Previous">
          ...
          <?php foreach($zype_pagination->links as $link){ ?>
            <?php if($zype_pagination->current == $link['url']){ echo '<strong>'; } ?>
              <li><a href="<?php zype_url('video'); ?>/page-<?php echo $link['url'].zype_search_and_sort_querystring(); ?>"><?php echo $link['title']; ?></a></li>
            <?php if($zype_pagination->current == $link['url']){ echo '</strong>'; } ?>
          <?php } ?>
          <?php if($zype_pagination->next){ ?>
            <li class="previous">
              <a href="<?php zype_url('video'); ?>/page-<?php echo $zype_pagination->next.zype_search_and_sort_querystring(); ?>" aria-label="Next">
